...
Fixpoint checkAll (ass : Assumption) (pl : PreProof) (c : Conclusion): bool :=
  match pl with
  | [] => true
  | p :: l => andb (isCorrectLastStep ass c p) (checkAll ass l c)
  end.

Definition isCorrect (ass : Assumption) (pl : PreProof) : bool :=
  let c := conclusions ass pl in
  checkAll ass pl c.
...
isCorrect ass0 (ass n :: p) = true -> ltb n (length ass0) = true.
Proof.
intros.
unfold isCorrect in H.
...
Lemma successorConclusions : forall (ass0 : Assumption) (a : ProofStep) (p : PreProof),
...
+ simpl.
  reflexivity.
+ rewrite successorConclusions.
  simpl.
  apply successor.
  assumption.
Qed.

Lemma ltb_length_S : forall (i : nat) (p : list nat),
  ltb i (length p) = ltb i (S (length p)).
Proof.
  intros i p.
...
Lemma checkall_resolution1 : forall (ass0 : Assumption) (p : PreProof) (c : Conclusion) (i j: nat) (k : clause),
isCorrect ass0 (res i j k :: p) = true -> ltb i (length p) = true.
Proof.
intros.
unfold isCorrect in H.
unfold checkAll in H.
...
Lemma checkall_resolution2 : forall (ass0 : Assumption) (p : PreProof) (c : Conclusion) (i j: nat) (k : clause),
isCorrect ass0 (res i j k :: p) = true -> ltb j (length p) = true.
Proof.
intros.
unfold isCorrect in H.
unfold checkAll in H.
apply andatomic1 in H.
apply andatomic2 in H.
apply andatomic1 in H.
destruct H.
Admitted.

Lemma checkall_resolution3 : forall (ass0 : Assumption) (p : PreProof) (c : Conclusion) (i j: nat) (k : clause),
isCorrect ass0 (res i j k :: p) = true -> 
isRes (findConclusion i c) (findConclusion j c) k = true.
Proof.
intros.
unfold isCorrect in H.
unfold checkAll in H.
...
Compute isCorrect list_ass [step0; step1; step2].
...
Lemma length_correct : forall (X : Type) (l : list X),
