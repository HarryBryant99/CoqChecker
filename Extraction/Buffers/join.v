...
CodeGen SourceFile "buffers/join.c".
...
CodeGen InductiveType string => "string".

CodeGen InductiveMatch string => "sw_string"
  | EmptyString => "default"
  | String => "case String_tag" "String_get_member_0" "String_get_member_1".

CodeGen InductiveMatch buffer => "sw_buffer" | Buf => "default" "Buf_get_member_0".
...
Definition join buf s := Buf (bufcontent buf ++ bufcontent s).

Compute join (create "a") (create "b").

Fixpoint equal s1 s2 : bool :=
 match s1, s2 with
 | EmptyString, EmptyString => true
 | String c1 s1', String c2 s2' => Ascii.eqb c1 c2 && equal s1' s2'
 | _,_ => false
 end.

Compute equal (bufcontent (create "a")) (bufcontent (create "a")).

Definition string_dec : forall s1 s2 : string, {s1 = s2} + {s1 <> s2}.

(*
Definition isUniqueBuf a b := 
if (equal (bufcontent a) (bufcontent b)) then a
else join a b.
*)

Definition isUniqueBuf a b := 
if (string_dec (bufcontent a) (bufcontent b)) then a
else join a b.

Definition joinBuffers a b := join a b.

Compute isUniqueBuf (create "aba") (create "aab").

CodeGen Gen isUniqueBuf.

CodeGen Linear buffer.
CodeGen StaticFunc sprintf _ "%s".
CodeGen StaticFunc create.
CodeGen StaticFunc equal.
CodeGen StaticFunc join.
(*CodeGen StaticFunc string_dec _ "%s + %s".*)
CodeGen StaticFunc isUniqueBuf.
CodeGen StaticFunc joinBuffers.
