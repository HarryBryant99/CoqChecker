...
let test_resolution () =
  ...
  let c2 = Cons (create_literal "b", Nil) in
  let l1 = create_literal "a" in
  let l2 = create_literal "b" in
  let f = Cons (c1, Cons (c2, Nil)) in
  let s1 = compute_subsumption c1 c1 f Nil in
  let s2 = compute_subsumption c2 c2 f s1 in

  let r1 = compute_resolution c1 l2 f s2 in
  let r2 = compute_resolution c2 l1 f s2 in

  let ures1 = is_resolution_complete c1 l2 f s2 in
  let ures2 = is_resolution_complete c2 l1 f ures1 in

  assert (mempu (Mk_fcp (f, r1)) ures2 = true);
  assert (mempu (Mk_fcp (f, r2)) ures2 = true);

  print_endline "Resolution tests passed!"
