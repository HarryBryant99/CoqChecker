...
let test_resolution () =
  ...
  let c2 = Cons (create_literal "~a", Nil) in
  let l1 = create_literal "a" in
  let l2 = create_literal "~a" in
  let f = Cons (c1, Cons (c2, Nil)) in
  let s1 = compute_subsumption c1 c1 f Nil in
  let s2 = compute_subsumption c2 c2 f s1 in

  let r1 = compute_resolution c1 l1 f s2 in

  let ures1 = is_resolution_complete c1 l1 f s2 in

  print_endline ("Result of r1: " ^ clause_to_string r1);
  print_endline ("Result of ures1: " ^ list_of_ures_to_string ures1)
