...
Theorem is_unsat_correct : forall (ass : Assumption) (s : Step) (p : PreProof),
  check ass s p = true ->
  lastconclusion s = true ->
  entails ass [].
Proof.
...
Theorem unsatisfiability_from_last_empty_clause :
  forall (ass : Assumption) (s : Step) (p : PreProof) (f : formula),
    check ass s p = true ->
    lastconclusion s = true ->
    unsatisfiable ass.
Proof.
  intros ass s p f Hcheck Hlast.
  unfold unsatisfiable.
  intros m Hmodels.
  (* We need to show that a model `m` satisfying `ass` leads to a contradiction *)
  unfold lastconclusion in Hlast.
  induction p as [| step tl IH].
  - (* Base case: p is empty *)
    simpl in Hlast. admit. (*discriminate.*)
  - (* Inductive step: non-empty proof *)
    simpl in Hcheck.
    destruct (correctconclusion step ass s (step :: tl)) eqn:Hcorrect.
    + (* The step is correct *)
      destruct step.
      * (* Case: step is an assumption *)
        simpl in Hcheck. simpl in Hlast.
        apply IH; auto.
        admit.
      * (* Case: step is a resolution *)
        simpl in Hcheck. simpl in Hlast.
        apply IH; auto.
        admit.
    + (* The step is incorrect, which contradicts Hcheck *)
      discriminate Hcheck.
Qed.
