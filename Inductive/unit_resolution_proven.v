...
assert (model_property m) as H2.
assumption.

assert (model_property m) as H7.
assumption.
...
Lemma five :
  ...
  (consclause (l' :: (remove eq_dec l ls))).
Proof.
intros.
unfold remove_literal_from_clause.
simpl.
...
destruct (eq_dec l l') as [H_eq | H_neq].
- (* Case where l = l' *)
  contradiction.
- (* Case where l <> l' *)
  reflexivity.
...
(*      specialize (IHls Hm_c').*)
      ...
      assert (models_clause m (consclause (l' :: remove eq_dec l ls))).
...
Lemma entailment_models :
  forall (f : formula) (c : clause) (l : literal),
    ...
    (*was opposite l but Anton proof changed it to l*)
Proof.
  intros f c l.
